...
const RATE_LIMIT_REQUESTS = 3; // Conservative limit for cancellation
 ...
 * 
 * This endpoint cancels a user's PRO subscription at the end of the billing period.
 * The user retains access until their current billing period expires.
 */
    ...
    return NextResponse.json(
      {
        ...
        metadata: {
          ...
          access_until: subscription.current_period_end ? new Date(subscription.current_period_end * 1000).toISOString() : null,
    ...
    return NextResponse.json(
      {
        ...
        request_id: `cancel_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      },
      { status: 500 }
    );
  }
...
export async function GET() {
  return NextResponse.json(
    {
      status: 'healthy',
      service: 'stripe-subscription-cancellation',
